    ...
    ;(async () => {
      ...
      if (
        ...
        objectKeyHasValue(
          ...
          String(category + parameter + map)
        )
      ) {
        setRecordedData(
          recordedDataDictionary[String(category + parameter + map)].slice(
            -(8 * heatMapItems.length)
          )
        )
      } else {
        try {
            ...
            await readCollection(
              ...
              query(
                ...
                where(
                  'date',
                  '>=',
                  Timestamp.fromDate(oldest?.date.toDate() ?? new Date())
                ),
                where('map', '==', map),
                orderBy('date', 'desc')
  ...
  useEffect(() => {
    ...
    setDateFilterOptions(
      getUniqueMonthsAndYears(
        recordedDataDictionary[String(category + parameter + map)]
      )?.reverse()
    )
  }, [recordedData])
  const State = {
...
export const predict = (input, parameter) => {
  return fetch(`${API_URI}/predict/${parameter?.toLowerCase()}`, {
    // mode: "no-cors",
    method: 'POST',
